...
class Vehicle(Loggable):
    ...
    LENGTH = global_val.LENGTH
    """ Vehicle length [m] """
    WIDTH = global_val.WIDTH
    """ Vehicle width [m] """
    DEFAULT_VELOCITIES = [23, 25]
    """ Range for random initial velocities [m/s] """
    MAX_VELOCITY = global_val.v_max
    """ Maximum reachable velocity [m/s] """
    MIN_VELOCITY = global_val.v_min
    """ Minimun reachable velocity [m/s]"""
    ...
    def step(self, dt):
        """
            ...
            Integrate a modified bicycle model with a first-order response on the steering wheel dynamics.
            If the vehicle is crashed, the actions are overridden with erratic steering and braking until complete stop.
            The vehicles current lane is updated.
    ...
    def to_dict_acc_training(self, origin_vehicle=None):
        d = {
            'x': self.position[0],
