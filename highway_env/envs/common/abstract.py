...
class AbstractEnv(gym.Env):
    ...
    ACTIONS = global_val.ACTIONS  # AV action space.
    BV_ACTIONS = global_val.BV_ACTIONS  # BV action space.
    ...
    def _simulate(self, mode = "NDD",action=None):
        """
        ...
        for k in range(int(self.SIMULATION_FREQUENCY // self.config["policy_frequency"])):
            if action is not None and \
                ...
                new_action = []
                for act in action:
                    new_action.append(self.ACTIONS[act])
                self.road.act(new_action)
            self.vehicle.act()
            self.road.act()
