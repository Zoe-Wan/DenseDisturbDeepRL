...
def get_nddprossi(state,action):
    matrix = [
        [0.0051,0.0082,0.0161,0.0264],
        [0.0051,0.0082,0.0161,0.0264],
        [0.0051,0.0082,0.0161,0.0264],
        [0.0051,0.0082,0.0161,0.0264],
        [0.0006,0.0025,0.0133,0.0301],
        ]
...
def get_nddaction(state,objidx):
    matrix = [
        [0.0051,0.0082,0.0161,0.0264],
        [0.0051,0.0082,0.0161,0.0264],
        [0.0051,0.0082,0.0161,0.0264],
        ...
        [0.9675,0.9532,0.9108,0.8528]
        ]
    matrix = np.array(matrix)
    objdis = state[objidx*2]*120
        ...
        while not done:
            ...
            if len(action_mask)>0:
                if args.nature:
                    ...
                    disturb_action =  get_nddaction(state,i)
                    action_logprob = math.log(get_nddprossi(state,disturb_action))
                elif args.test_flag:
                ...
                disturbed_obs,disturb_action = disturber.disturb(obs.cav_observation, disturb_action, indexes)
            ...
            if ori_CAV_action == CAV_action:
                disturb_action=None
                ndd_prossi = 1
                action_logprob=0
            # else:
            #     print(CAV_action)
            #     print(ori_CAV_action)
            #     breakpoint()

            if disturb_action!=None:
                # 给一个视觉效果（染黑）
                # print('扰动'+str(disturb_action//5))
                if disturb_action%6!=5:
                    vid = indexes[disturb_action//6]
                    if vid is not None:
                        v = env.cav_obs_vehs_list[vid-1]
                        v.disturbed = disturb_action%6+1
                        # print(disturb_action%6+1)
            else:
                ndd_prossi = 1
                action_logprob=0
            ...
            if info["scene_type"] == "AV-Crash":
                ...
                reward = args.reward*(1-sum_weight*10)
            ...
            if len(action_mask)>0 and disturb_action is not None:
